 ...
 */
      ...
      Allocator::internalAllocateArray(
        ...
        std::size_t size = sizeof(ArrayType);
        ArrayType *array;
...
//        SET_VARSIZE(array, size);
        ...
        array->dims = static_cast<int *>(palloc(sizeof(int) * Dimensions));
        array->bounds = static_cast<int *>(palloc(sizeof(int) * Dimensions));
        array->dataoffset = 0;
        array->elemtype = TypeTraits<T>::oid;
        for (std::size_t i = 0; i < Dimensions; ++i) {
          ARR_DIMS(array)[i] = static_cast<int>(inNumElements[i]);
          ARR_LBOUND(array)[i] = 0;
        }
        size = sizeof(T) * numElements;
        void * data = palloc0(size);
        array->data = PointerGetDatum(data);
 ...
 */
      ...
      Allocator::internalRePalloc(void *inPtr, size_t inSize) const {
        ...
        /* Precondition: inSize <= std::numeric_limits<size_t>::max() - 16 */
        const size_t size = inSize + 16;
        void *raw = realloc(unaligned(inPtr), size);
